...
contract FinTabToken is SmartToken {
    ...
    function setTeamAddress(address addr, bool _status) public onlyOwner {
        require(addr != 0x0);
        teamAddresses[addr] = _status;
    }

    function setBurner(address addr, bool state) public onlyOwner {
        require(addr != 0x0);
    ...
    function transfer(address _to, uint _value, bytes _data) public
        transfersAllowed canTransfer(msg.sender) returns (bool success)
    {
        if (isContract(_to)) {
    ...
    function transfer(address _to, uint _value) public transfersAllowed canTransfer(msg.sender) returns (bool success) {
        //standard function transfer similar to ERC20 transfer with no _data
        //added due to backwards compatibility reasons
        bytes memory empty;
        if (isContract(_to)) {
    ...
    function transferFrom(address _from, address _to, uint _value) public
        transfersAllowed canTransfer(_from) returns (bool success)
    {
    ...
    // Burn tokens
    function burn(uint _value) public canBurnTokens(msg.sender) returns (bool success) {
        return super.burn(_value);
    }

    // Burn tokens on _from behalf
    function burnFrom(address _from, uint _value) public
        onlyOwner  canBurnTokens(msg.sender) returns (bool success)
    {
